...
- The sync now also fetches text for the Imageshop language of every Craft site, and `mapDocumentFields()` adds a language block that was missing from a stored value, so a site added after an image was picked receives its text on the next sync.
