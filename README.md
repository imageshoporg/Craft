...
```
php craft plugin/install imageshop-da
```
...
```bash
php craft migrate --plugin=imageshop-da
```
...
```
GET /actions/imageshop-da/permalink/get-hq-url
```
...
document.querySelectorAll('.gallery figure').forEach(function(fig) {
    fig.addEventListener('click', function() {
        ...
        if (img.documentId && !hqCache[img.documentId]) {
            fetch('/actions/imageshop-da/permalink/get-hq-url?documentId=' + img.documentId)
